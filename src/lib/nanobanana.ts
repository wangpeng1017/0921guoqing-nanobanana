...
export const createNanobananaPrompt = (styleType: string) => {
  const prompts = {
    ...
    nostalgic: `Precise face swap. Please perfectly replace the child's face in the reference image with the face from my uploaded child's photo. Requirements: 1. Completely preserve the vintage background, nostalgic atmosphere, retro clothing, and all decorative elements from the reference image. 2. Match the vintage hairstyle and classic makeup style from the reference image for my child's head. 3. Core requirement: Ensure 100% similarity of my child's facial features and appearance, with no uncharacteristic changes. The overall style should be a high-definition, nostalgic, vintage-themed children's portrait with warm retro tones.`
  };
